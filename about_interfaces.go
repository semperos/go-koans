...
func aboutInterfaces() {
	...
	assert(runner(bob) == _Runner) // conformed interfaces need not be declared, they are inferred
	...
	assert(bob.milesCompleted == _Int)   // bob is affected by running in his own unique way (probably fatigue)
	assert(rspec.executionCount == _Int) // rspec can run completely differently than bob, thanks to interfaces
}
